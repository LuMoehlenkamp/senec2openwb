...
//     std::cout << "[" << std::put_time(&tm, "%H:%M:%S") << "] " << name_
//               << " failure ";
...
//               << (tok ? tok->get_message_id() : -1) << " (only for QOS > 0)"
//               << std::endl;
//   }
...
//     std::cout << "\n[" << std::put_time(&tm, "%H:%M:%S") << "] "
//               << "Sending 🖐  message with QOS=" << QOS
//               << " on topic 'hello'... " << std::endl;
// #ifdef MQTT5
//     auto pubproperties = mqtt::properties();
//     pubproperties.add(
//         {mqtt::property::MESSAGE_EXPIRY_INTERVAL,
//         MY_MESSAGE_EXPIRY_INTERVAL});
//     auto msg = mqtt::message::create("hello",
//                                      "Cedalo is awesome! (paho c++ client
//                                      v5)", QOS, true, pubproperties);
// #else
//     auto msg = mqtt::make_message(
//         "hello", "Cedalo is awesome! (paho c++ client v3.1.1)", QOS, true);
// #endif
...
//       std::cout << "...OK, sent message with id: " << tok->get_message_id()
//                 << std::endl;
//     } else {
...
// public:
//   // constructor
//   mqtt_client_callback(mqtt::async_client &cli, mqtt::connect_options
//   &connOpts)
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // std::signal(SIGINT, S2O::PowerControl::SignalHandler);
    //   std::signal(SIGTERM, S2O::PowerControl::SignalHandler);
  ...
  //     if (!tstore) {
  //       cerr << "The trust store file does not exist: " << TRUST_STORE <<
  //       endl; return 1;
  //     }
  ...
  //   // --> see source code header: paho.mqtt.cpp/src/mqtt/properties.h for
  //   all
  //   // options
  ...
  //   auto willmsg =
  //       mqtt::message(LWT_TOPIC, LWT_PAYLOAD, QOS, true, willproperties);

  //   // client properties
  //   auto properties = mqtt::properties();
  //   properties.add(
  //       {mqtt::property::SESSION_EXPIRY_INTERVAL,
  //       MY_SESSION_EXPIRY_INTERVAL});
